...
class FileUploadResponse(BaseModel):
    ...
    message: str = Field(..., description="A message indicating the outcome of the processing.")
    total_chunks_processed: int = Field(..., description="The number of text chunks successfully processed and upserted.")
    document_id: Optional[str] = Field(None, description="A unique ID assigned to the uploaded document, used for linking chunks.")
...
class ChatLogEntryModel(BaseModel):
    interaction_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Primary key for the log entry.")
    ...
    retrieved_doc_ids: Optional[List[str]] = Field(None, description="List of document/chunk IDs used for context.")
    feedback: Optional[int] = Field(None, description="User feedback (e.g., +1 for good, -1 for bad).")
...
class SessionDetailModel(BaseModel):
    ...
    last_interaction_time: datetime = Field(..., description="Timestamp of the last interaction in this session.")
    total_messages: int = Field(..., description="Total number of message pairs (user + AI) in this session.")
